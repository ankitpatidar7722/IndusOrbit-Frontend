...
function TesterDashboard({ uid }: { uid: number }) {
  ...
  const actions = d && (
    <>
      {s === "PendingQC" && <Button variant="action-save" onClick={() => drawer.act(() => pmApi.startTester(d.pointID, uid))} disabled={dis}><Play size={15} style={ic} /> Start Testing</Button>}
      {s === "In-Testing" && <Button variant="action-save" onClick={() => drawer.act(() => pmApi.completeTesting(d.pointID, uid))} disabled={dis}><Square size={15} style={ic} /> Complete Testing</Button>}
      {s === "Testing-Completed" && <Button variant="action-save" onClick={() => drawer.act(() => pmApi.verifyClose(d.pointID, uid, rk))} disabled={dis}><CheckCircle2 size={15} style={ic} /> Verify &amp; Close</Button>}
      {(s === "PendingQC" || s === "In-Testing" || s === "Testing-Completed") &&
        <Button variant="action-delete" onClick={() => drawer.act(() => pmApi.testerReopen(d.pointID, uid, rk))} disabled={dis}><RotateCcw size={15} style={ic} /> Reopen for Dev</Button>}
    </>
  );
